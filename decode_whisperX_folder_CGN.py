...
# Examples:
# python3 decode_whisperX_folder_CGN.py audio/examples tiny 0 output/nl /vol/tensusers5/ctejedor/whisper/models 0
# nohup time python3 decode_whisperX_folder_CGN.py /vol/bigdata/corpora2/CGN2/data/audio/wav large-v2 nl /vol/tensusers4/ctejedor/shared/cgn-whisperx /vol/tensusers5/ctejedor/whisper/models 0 &
# nohup time python3 decode_whisperX_folder_CGN.py audio/en tiny en output/en /vol/tensusers5/ctejedor/whisper/models 0 &
...
import glob
fn = glob.glob(MY_DIR+'/**',recursive=True)
onlyfiles = [x for x in fn if 'nl/fn' in x and extension in x]
#print(len(onlyfiles))
...
for file in onlyfiles:
    ...
    if not Path(join(OUTPUT_DIR,filebase+'.txt')).is_file() or not Path(join(OUTPUT_DIR,filebase+'.json')).is_file():
        ...
        try:
            ...
            print("**-** Error loading the audio file --> ",str(counter), file, " not possible to write results ...")
            print(e)
